...
# ====================================================
def initial_hmms_customers(num_proc, customers, N, M, size, overlap, epsilon, minfloat):
...
# =====================
# 23. Merging PDF files 
# =====================
def merge_pdfs(list_files, file_output):
    merger = PdfMerger()
...
def main():
    ...
    # Temporary files of each processor
    file_hmm = f"{dir_tmp}/hmm_customers_{num_proc}.dat"
    csv_omega_bar = f"{dir_tmp}/omega_bar_{num_proc}.csv"
    csv_omega = f"{dir_tmp}/omega_{num_proc}.csv"
    csv_instability_rates_and_levels = f"{dir_tmp}/instability_{num_proc}.csv"
    my_pdf_profiles = f"{dir_tmp}/profiles_{num_proc}.pdf"
    ...
    if num_proc == 0:
        ...
        print(f"\nData access time: {end_step - start_step:g} s")
        start_step = time.perf_counter()
    ...
    # Creating the customer local file containing the curves omega_bar of the customers
    f = open(csv_omega_bar, "w", encoding="utf-8")
    f.close()
	
    # Creating the customer local file containing the curves omega of the customers
    f = open(csv_omega, "w", encoding="utf-8")
    f.close()
	
    # Construction of the initial HMMs of the local customers
    initial_hmms = initial_hmms_customers(num_proc, customers_local, nb_states, nb_symbols, window_size, overlap, epsilon, minfloat)

    # Local customer HMM training
    features = train_hmms_customers(customers_local, initial_hmms, maxiter, minfloat, r, tolerance, file_hmm, csv_omega_bar, csv_omega)

    # Calculating local customers instability levels
    instability_rates_SASI, instability_levels, instability_rates = instability_rate_and_level_customers(customers_local, features, csv_instability_rates_and_levels)

    # Static PDF visualization of local customers instability levels in R^2
    visualize_customers_profile_pdf(instability_rates_SASI, instability_rates, customers_local, features, instability_levels, date_start_str, date_end_str, window_size, overlap, my_pdf_profiles)

    # Aggregation of local results
    local_results = (customers_local, features, instability_rates_SASI, instability_levels, instability_rates, my_pdf_profiles)


    # Transmission of local results to master
    if num_proc != 0:
        # ======
        # SLAVES
        # ======
        ...
        customers_total = []
        features_total = []
        instability_rates_SASI_total = []
        instability_levels_total = []
        instability_rates_total = []
        list_files_pdf_profile = []

        # inserting the master's results
        customers_total += customers_local
        features_total += features
        instability_rates_SASI_total += instability_rates_SASI
        instability_levels_total += instability_levels
        instability_rates_total += instability_rates
        list_files_pdf_profile.append(my_pdf_profiles)
        ...
        for p in range(1, nb_procs):
            ...
            customers_p, features_p, instability_rates_SASI_p, instability_levels_p, instability_rates_p, file_pdf_profile_customers_p = local_results_p

            customers_total += customers_p
            features_total += features_p
            instability_rates_SASI_total += instability_rates_SASI_p
            instability_levels_total += instability_levels_p
            instability_rates_total += instability_rates_p
            list_files_pdf_profile.append(file_pdf_profile_customers_p)
			
        end_step = time.perf_counter()
        print(f"\nHMM-based + score computation time: {end_step - start_step:g} s")

    # ==============
    # ALL PROCESSORS
    # ==============

    # Synchronization before merging local PDF files
    comm.Barrier()
    ...
    if num_proc == 0:
        ...
        # Merging local PDF files visualizing customer profile
        merge_pdfs(list_files_pdf_profile, pdf_profiles)

        # Deleting local PDF files
        for file_pdf in list_files_pdf_profile:
            os.remove(file_pdf)

        # Dynamic HTML visualization of customer profile in R^2
        visualize_customers_profile_html(instability_rates_SASI_total, instability_rates_total, customers_total, features_total, instability_levels_total, date_start_str, date_end_str, window_size, overlap, html_profiles)

        # Static PDF visualization of customer instability levels in R^2
        visualize_customers_pdf(instability_rates_SASI_total, instability_levels_total, nb_instability_levels, date_start_str, date_end_str, window_size, overlap, pdf_instability_levels)

        # Dynamic HTML visualization of customer instability levels in R^2
        visualize_customers_html(instability_rates_SASI_total, instability_rates_total, instability_levels_total, nb_instability_levels, customers_total, date_start_str, date_end_str, window_size, overlap, html_instability_levels)
